...
P, polytope, is represented by A$\vec{x}$ $\leqslant$ $\vec{b}$. Let $\vec{x}$ $\in$ 
$\mathbb{R}^{n}$, so $\vec{x}$ is a point within P if A$\vec{x}$ $\leqslant$ $\vec{b}$ holds.
More specifically, let A be an m x n matrix, and $\vec{b}$ a series of m constants, such that 
...
Suppose $\vec{x}$ exists at the border of P, then for some i, specifically the i that represents the facet on which $\vec{x}$ exists, a$_{i1}$x$_{1}$ + ... + a$_{in}$x$_{n}$ = b$_{i}$. Ideally if we are to find some centre $\vec{y}$ $\in$ $\mathbb{R}^{n}$ in P such that the largest ball can fit inside, it would follow that some point on the ball must intersect with at least one of the facets of P.\\

The LP must be designed so that we find a centre of the ball $\vec{y}$ in P, such that all points on the surface of the ball that are in the plane where P exists are also in P. Additionally, one or more of these points intersects with at least one of the facets.\\

Suppose that $\vec{a}$$_{i}$ = [a$_{i1}$, ..., a$_{in}$] is a unit vector for some i, such that $\lVert$$\vec{a}$$_{i}$$\rVert$ = 1. Indeed, $\vec{a}$$_{i}$ is the unit vector normal to the facet represented by a$_{1}$x$_{1}$ + ... + a$_{n}$x$_{n}$ = b$_{i}$. To discover if the ball at center $\vec{y}$ with radius r $\in$ $\mathbb{R}$ can fit in P we check for each facet i if the surface point $\vec{z}_{i}$ = ($\vec{a}$$_{i}$*r + $\vec{y}$), that is perpendicular to the facet i, is within P. In order to find the largest such ball, the LP must be designed to calculate maximum radius r with centre $\vec{y}$ given constraints a$_{i1}$z$_{i1}$ + ... + a$_{in}$z$_{in}$ $\leqslant$ b$_{i}$, for all i.  \\

Thus, the LP is:\\
\textbf{maximize:} \\r\\
\textbf{subject to:} \\
$\vec{a}_{i} ~\cdot$ ($\vec{a}$$_{i}$*r + $\vec{y}$) $\leqslant$ b$_{i}$, $\forall$ i, 1 $\leqslant$ i $\leqslant$ m\\ r $\geqslant$ 0.\\
